...
While performance wasn't a number one concern for me when writing `plotscaper`, I found it can actually perform fairly well on moderately-sized data sets (thanks to the work of the very smart people who have optimized JavaScript engines like V8). 
...
On my machine, I found that the figure with 50,000 cases is a bit sluggish, but still fast enough to feel "interactive" rather than a "slideshow". Your mileage may vary.

Note that most of the slowdown is due to rendering rather than computation. In the scatterplot, we have to draw each of the 50,000 points each time selection happens, hence why it is not as snappy as with more modestly sized data sets. Same goes for parallel coordinates plots. If instead you only use plots which show aggregate summaries of the data, (e.g. barplot, histogram, fluctuation diagram, etc...) then the interactions like selection should happen a lot faster.
...
sc <- screenshot()
image <- image_read(sc)
image2 <- image_crop(image, geometry_area(800, 600, 1420, 50))
