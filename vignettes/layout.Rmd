...
The matrix can be of any size, the only requirements is that the plot areas must be contiguous, meaning that e.g. this would not be valid:
...
layout <- matrix(c(
  2, 1,
  1, 2
), ncol = 2) # How do you draw this?
```

Also, all of the plots should be ideally included in the layout. If they're not, you'll get a pop-up warning, and the layout might not work as well.

You can call the `set_layout` function from inside a running R session too with `scene |> set_layout(...)` (and you will get a client-side update to the current figure).
