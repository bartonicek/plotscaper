...
#' @param mapping A list which declares how variables in the data (belonging to scene) map to visual aesthetics/encodings
#'
#' @export
#'
#' @examples
...
ps_wrapper_plot <- function(scene, type, mapping, height = NULL, width = NULL) {

  if (type %in% c("scatter") & is.null(mapping$y)) stop("Please supply a valid y mapping")
  if (!is.null(height) && !is.null(width)) scene$x$dims <- append(scene$x$dims, list(c(width = width, height = height)))
